...
const renderBooks = (bookArray) => {
  ...
  let list = "<p><strong>List of Books</strong> </p> <ul>";

  for (let i = 0; i < bookArray.length; i++) {
    list += `<li><b>${bookArray[i].title}</b><br> Author:
    ${bookArray[i].author} <br></br> </li>`
  }
...
const detailedView = (bookArray) => {
  ...
  for (let i = 0; i < bookArray.length; i++) {
    ...
    list += `<li class="theBook"><b>${book.title}</b><br> Author:
    ${book.author} <br></br>
...
const render = () => {
  if (display === 'detail') {
    detailedView(globalData.books);
  }
  else {
    renderBooks(globalData.books);
  }
}
...
window.onload = () => {
  ...
  fetch('books.json')
    ...
    .then(data => {
      globalData = data;  //THIS IS Important to declare a global Data so that render() can be used.

      // renderBooks(data.books); //remember Object then calls the array .books.
      //detailedView(data.books);
      render();
    })
